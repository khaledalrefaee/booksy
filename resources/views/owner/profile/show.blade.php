                    ...
                    <div class="d-flex align-items-center gap-4 mb-5 pb-4 border-bottom">
                        <div class="position-relative flex-shrink-0">
                            <img id="avatar-preview"
                                 src="{{ $owner->avatar ? asset('storage/' . $owner->avatar) : 'https://ui-avatars.com/api/?name=' . urlencode($owner->name) . '&size=96&background=random' }}"
                            ...
                            <label for="avatar-input"
                                   ...
                                   style="width:28px;height:28px;cursor:pointer;" title="{{ __('Change avatar') }}">
                                <i data-feather="camera" style="width:14px;height:14px;"></i>
                            </label>
                        </div>
                        <div>
                            <h5 class="mb-1 fw-bold">{{ $owner->name }}</h5>
                            <p class="text-muted mb-0">{{ $owner->email }}</p>
                        </div>
                    </div>

                    <form method="POST" action="{{ route('owner.profile.update') }}" enctype="multipart/form-data" novalidate>
                        @csrf
                        @method('PUT')

                        {{-- Hidden avatar input (triggered by the camera button) --}}
                        <input type="file" id="avatar-input" name="avatar" accept="image/*" class="d-none">

                        {{-- Info section --}}
                        <h6 class="fw-semibold text-muted text-uppercase small mb-3">{{ __('Personal information') }}</h6>

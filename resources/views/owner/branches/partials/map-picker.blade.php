...
document.addEventListener('DOMContentLoaded', function () {
    ...
    /* ── Debounced real-time search (fires 400 ms after last keystroke) */
    var debounceTimer = null;
    var lastQuery     = '';
    var busy          = false;
    var lastCallAt    = 0;           // guard against Nominatim 1 req/s limit
    ...
    /* Nominatim fetch — shared by both real-time and button */
    function fetchSuggestions(q) {
        if (busy) return;
        var now = Date.now();
        if (now - lastCallAt < 1050) {
            /* too soon — retry after the cooldown */
        ...
        var url = 'https://nominatim.openstreetmap.org/search'
            + '?format=json&limit=6&addressdetails=1&bounded=0'
            + '&q=' + encodeURIComponent(q);

        fetch(url, { headers: { 'Accept': 'application/json', 'Accept-Language': uiLang } })
    ...
    /* Button / Enter: pick first result if list already open, else fetch */
    function runSearch() {
        var items = resultsEl.querySelectorAll('.list-group-item');

...
@php
    ...
    $statusClass = match ($campania->status) {
        'active' => 'success',
        'suspended' => 'danger',
        default => 'warning',
    };
...
<div class="page-content">
    <div class="d-flex justify-content-between align-items-start flex-wrap gap-3 mb-4">
        <div>
            <h4 class="mb-2">{{ $campania->localizedName() }}</h4>
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb mb-0">
                    <li class="breadcrumb-item"><a href="{{ route('owner.dashboard') }}">{{ __('Dashboard') }}</a></li>
                    <li class="breadcrumb-item"><a href="{{ route('owner.campanias.index') }}">{{ __('Campanias') }}</a></li>
                    <li class="breadcrumb-item active">{{ $campania->localizedName() }}</li>
                </ol>
            </nav>
        </div>
        <a href="{{ route('owner.campanias.index') }}" class="btn btn-outline-secondary rounded-pill">
        ...
        <div class="col-lg-8">
            <div class="card border-0 shadow-sm rounded-4 h-100">
                <div class="card-body p-4 d-flex flex-wrap gap-4 align-items-center">
                    @if ($campania->logo)
                        <img src="{{ asset('storage/'.$campania->logo) }}" alt="" class="rounded-3 border" width="80" height="80" style="object-fit:cover;">
                    @else
                    ...
                    <div class="flex-grow-1">
                        <div class="d-flex flex-wrap align-items-center gap-2 mb-2">
                            <h5 class="mb-0">{{ $campania->localizedName() }}</h5>
                            <span class="badge rounded-pill bg-{{ $statusClass }}">{{ __($campania->status) }}</span>
                        </div>
                        <p class="text-muted mb-1"><span class="fw-semibold text-body">{{ __('Email') }}:</span> {{ $campania->email }}</p>
                        <p class="text-muted mb-1"><span class="fw-semibold text-body">{{ __('Phone') }}:</span> {{ $campania->phone ?: '—' }}</p>
                        <p class="text-muted mb-0"><span class="fw-semibold text-body">{{ __('Category') }}:</span> {{ $campania->category?->localizedName() ?? '—' }}</p>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="tab-content">
        ...
        <div class="tab-pane fade show active" id="tab-branches" role="tabpanel">
            @forelse ($campania->branches as $branch)
            ...
            <div class="card border-0 shadow-sm rounded-4 overflow-hidden">
                <div class="table-responsive">
                    <table class="table table-hover align-middle mb-0">
                        ...
                        <tbody>
                            @forelse ($campania->employees as $employee)
                                <tr>
            ...
            <div class="card border-0 shadow-sm rounded-4 overflow-hidden">
                <div class="table-responsive">
                    <table class="table table-hover align-middle mb-0">
                        ...
                        <tbody>
                            @forelse ($campania->appointments as $appointment)
                                @php
                ...
                @if ($stats['appointments'] > $campania->appointments->count())
                    <div class="card-footer bg-transparent border-0 text-muted small">
                        {{ __('Showing latest :count of :total appointments.', ['count' => $campania->appointments->count(), 'total' => $stats['appointments']]) }}
                    </div>
                @endif
            </div>
        </div>
            ...
            <div class="card border-0 shadow-sm rounded-4 overflow-hidden">
                <div class="table-responsive">
                    <table class="table table-hover align-middle mb-0">
                        ...
                        <tbody>
                            @forelse ($campania->waitlistEntries as $entry)
                                <tr>

...
<div class="page-content">
    <div class="d-flex justify-content-between align-items-center flex-wrap grid-margin">
        <div>
            <h4 class="mb-1">{{ __('Edit service') }}</h4>
        ...
        <a href="{{ route('company.branches.services.index', $service->branch) }}" class="btn btn-outline-secondary rounded-pill">
        ...
        <div class="col-md-8 col-xl-7">
            <div class="card border-0 shadow-sm rounded-4">
                <div class="card-body p-4">
                    <form method="POST" action="{{ route('company.services.update', $service) }}">
                        @csrf @method('PUT')
                        <div class="mb-3">
                            ...
                            <select id="service_category_id" name="service_category_id" class="form-select">
                                ...
                                @foreach($serviceCategories as $cat)
                                    <option value="{{ $cat->id }}" {{ old('service_category_id', $service->service_category_id) == $cat->id ? 'selected' : '' }}>{{ $cat->localizedName() }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="text" id="name_en" name="name_en" class="form-control @error('name_en') is-invalid @enderror" value="{{ old('name_en', $service->name_en) }}">
                                @error('name_en')<div class="invalid-feedback">{{ $message }}</div>@enderror
                            </div>
                            <div class="col-md-6 mb-3">
                                <label for="name_ar" class="form-label fw-semibold">{{ __('Name (AR)') }}</label>
                                <input type="text" id="name_ar" name="name_ar" dir="rtl" class="form-control" value="{{ old('name_ar', $service->name_ar) }}">
                            </div>
                        </div>
                        <div class="mb-3">
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                ...
                                <div class="input-group">
                                    <input type="number" id="price" name="price" class="form-control @error('price') is-invalid @enderror" value="{{ old('price', $service->price) }}" min="0" step="0.01">
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <div class="input-group">
                                    <input type="number" id="duration_minutes" name="duration_minutes" class="form-control @error('duration_minutes') is-invalid @enderror" value="{{ old('duration_minutes', $service->duration_minutes) }}" min="1">
                        ...
                        <button type="submit" class="btn btn-primary rounded-pill px-4">{{ __('Update service') }}</button>
                    </form>
                </div>
            </div>
        </div>
